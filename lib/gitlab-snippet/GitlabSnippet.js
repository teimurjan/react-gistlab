...
var GITLAB_SNIPPET_URL_REGEX = /^https:\/\/gitlab\.com\/.*\/([0-9a-z]*)/;
...
exports.getSnippetHTML = function (responseText) {
    ...
    return {
        css: cssHref,
        html: exports.unescapeHTML(html)
    };
};
exports.unescapeHTML = function (html) {
    return html
        .replace(/\\"/g, '"')
        .replace(/\\n/g, '')
        .replace(/\\\//g, '/');
...
var GitlabSnippet = (function (_super) {
    ...
    function GitlabSnippet(props) {
        ...
        _this.getSnippet = function () {
            var _a = _this.props, corsProxyURL = _a.corsProxyURL, snippetURL = _a.url;
            var url = "" + corsProxyURL + snippetURL + ".js";
            var request = new XMLHttpRequest();
            request.open('GET', url);
            request.send();
            request.onreadystatechange = function () {
                if (request.readyState === 4 && request.status === 200) {
                    var _a = exports.getSnippetHTML(request.responseText), css = _a.css, html = _a.html;
                    _this.addSnippetToDOM(css, html);
                }
            };
        };
        _this.addSnippetToDOM = function (css, html) {
            if (!utils_1.isDocumentDefined()) {
        ...
        _this.state = {
            isLoading: true,
            data: undefined,
            info: exports.getInfo(props.url)
        };
        return _this;
    }
